...
export const CheckboxImage = ({ imageUrl, onToggle }) => {
  const [isChecked, setIsChecked] = useState(false);

  const toggleCheckbox = () => {
    setIsChecked(!isChecked);
    onToggle(imageUrl, !isChecked);
  };
  ...
  return (
    <div className="flex items-center relative">
      <img
        ...
        className={
          isChecked
            ? "w-40 mr-4 cursor-pointer opacity-75 transition-all bg-red-500"
            : "w-40 mr-4 cursor-pointer transition-all bg-red-500"
        }
